...
const Notification_1 = __importStar(require("../models/Notification")); // <-- ZID HADA
// --- ZID IMPORTS L-JDAD ---
const notifications_1 = require("../utils/notifications");
const pubsub_2 = require("../utils/pubsub"); // <-- BDDEL HADA
// -------------------- Populates --------------------
...
function buildProjectFilter(permissions, userId) {
    ...
    if (permissions.includes('manage_own_tasks') || permissions.includes('upload_methodology')) {
        return {
            $or: [
...
exports.resolvers = {
    Query: {
        ...
        project: async (_, { id }, context) => {
            ...
            for (const p of teamPopulates)
                q = q.populate(p);
            const project = await q.exec(); // 7eyyedna .lean()
            if (!project) {
        ...
        tasksByProject: async (_, { projectId }, context) => {
            ...
            return tasksAgg.map(task => {
                ...
                if (fullTask?.finalUpload && !fullTask.finalUpload.uploadedBy) {
                    fullTask.finalUpload.uploadedBy = defaultUser;
                }
                return {
                    ...task,
                    id: task._id.toString(), // <-- FIX
                    v1Uploads: v1Uploads,
        ...
        myTasks: async (_, __, context) => {
            ...
            result.forEach((task) => {
                ...
                if (task.finalUpload && !task.finalUpload.uploadedBy) {
                    task.finalUpload.uploadedBy = defaultUser;
                }
            });
    ...
    Mutation: {
        ...
        assignDynamicProjectManager: async (_, { projectId, newPmId }, context) => {
            ...
            if (!isCandidate) {
                throw new apollo_server_errors_1.ApolloError('Forbidden: User is not an authorized candidate for Project Manager.', 'FORBIDDEN');
            }
            // 3. N-assigniw l-PM l-project
            const updatedProject = await Project_1.default.findByIdAndUpdate(projectId, { $addToSet: { projectManagers: newPmId } }, // Nzidohom Project Manager
            { new: true })
                .populate({ path: 'projectManagers', select: userSelect });
            if (!updatedProject) {
                throw new apollo_server_errors_1.ApolloError('Project not found or update failed.', 'NOT_FOUND');
            }
            // 4. L-Log w L-Notification
            const newPm = updatedProject.projectManagers.find(pm => pm._id.toString() === newPmId);
            await (0, logger_1.logActivity)(context.user.id, `Assigned new dynamic PM: ${newPm?.name || newPmId} to project: ${updatedProject.title}`, 'PROJECT_UPDATE', projectId);
        ...
        updateProject: async (_, { id, input }, context) => {
            ...
            const project = await Project_1.default.findByIdAndUpdate(id, { $set: input }, { new: true });
            if (!project)
                throw new apollo_server_errors_1.ApolloError('Project not found', 'NOT_FOUND');
        ...
        admin_assignProject: async (_, { input }, context) => {
            ...
            const project = await Project_1.default.findByIdAndUpdate(projectId, { projectManagers: projectManagerIds, preparationStatus: status, currentStage: 'ADMINISTRATIVE' }, { new: true }).populate({ path: 'projectManagers', select: userSelect });
            if (!project)
                throw new apollo_server_errors_1.ApolloError('Project not found', 'NOT_FOUND');
            await (0, logger_1.logActivity)({
                userId: context.user.id,
        ...
        pm_updateTaskStatus: async (_, { taskId, status }, context) => {
            if (!context.user)
                throw new apollo_server_errors_1.ApolloError('Not authenticated', 'UNAUTHENTICATED');
            const userRole = await Role_1.default.findById(context.user.role);
            if (!userRole.permissions.includes('update_workflow_stage') &&
            ...
            // --------------------------------
            pubsub_1.pubsub.publish(pubsub_2.TASK_UPDATED_EVENT, { taskUpdated: populatedTask });
            return populatedTask;
        },
        ...
        giveProposalAvis: async (_, { projectId, status, reason }, context) => {
            ...
            if (!userRole || userRole.name !== 'PROJECT_MANAGER') {
                throw new apollo_server_errors_1.ApolloError('Forbidden - Only PROJECT_MANAGER can give avis', 'FORBIDDEN');
            }
            const project = await Project_1.default.findById(projectId);
            if (!project)
                throw new apollo_server_errors_1.ApolloError('Project not found');
            ...
            if (!['ACCEPTED', 'NOT_ACCEPTED'].includes(status)) {
                throw new apollo_server_errors_1.ApolloError('Invalid avis status - must be ACCEPTED or NOT_ACCEPTED');
            }
            // Khzn l-avis
        ...
        cp_uploadEstimate: async (_, { projectId, fileUrl, originalFileName }, context) => {
            ...
            if (!allowedStatuses.includes(project.preparationStatus)) {
                throw new apollo_server_errors_1.ApolloError(`Impossible d'uploader l'estimation. Statut actuel : ${project.preparationStatus}`);
            }
    ...
    Subscription: {
        ...
        taskUpdated: {
            ...
            subscribe: () => pubsub_1.pubsub.asyncIterator(pubsub_2.TASK_UPDATED_EVENT),
        ...
        newNotification: {
            subscribe: (0, graphql_subscriptions_1.withFilter)(() => pubsub_1.pubsub.asyncIterator(pubsub_2.NEW_NOTIFICATION_EVENT), (payload, variables) => {
