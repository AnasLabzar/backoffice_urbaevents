...
import {
    ...
    IconPlus, IconClock, IconAlertTriangle, IconSearch,
    IconLoader, IconBriefcase, IconInfoCircle, IconChecklist,
    IconCloudUpload, IconFile, IconDownload, IconTrash, IconX,
    IconFlag, IconCalendar, IconAlertCircle, IconArrowRight,
    IconCheck, IconUserCircle, IconArrowLeft, IconMenu2
...
const GET_PROJECT_FULL_DETAILS = gql`
  query GetProjectFullDetails($projectId: ID!) {
    ...
    # Fetch all available users for team selection and task assignment
...
export default function ProductionPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // Construct Team Array for Display
    ...
    // Combine all available users for Task Assignment dropdown
                ...
                <div className="min-h-screen bg-muted/5 flex flex-col">
                    ...
                    <div className="flex-1 p-4 sm:p-6 md:p-8 max-w-[1800px] mx-auto w-full">
                        <Tabs defaultValue="tasks" className="flex flex-col h-full space-y-6">
                            ...
                            {/* TAB 1: TASKS (Advanced Inline Creator + Kanban) */}
...
// ------------------------------------------------------------------
// COMPONENT 1: PROFESSIONAL TASK CREATOR (Pro UI/UX & Responsive)
// ------------------------------------------------------------------
    ...
    const [createTask, { loading }] = useMutation(CREATE_TASK_MUTATION, {
        onCompleted: () => {
            toast.success("Tâche créée avec succès", {
                description: "L'assignation a été notifiée au membre de l'équipe."
            });
            // Reset form
            setDesc("");
            setAssignee("");
    ...
    return (
        <Card className={cn(
            ...
            <CardContent className="p-0">
                ...
                {/* 2. CONTROLS BAR (Visible when expanded) */}
                {isExpanded && (
                    <div className="bg-muted/30 border-t p-3 flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between animate-in fade-in slide-in-from-top-2">
                        ...
                        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 w-full lg:w-auto">
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                ...
                                <Select value={priority} onValueChange={setPriority}>
                                    <SelectTrigger className={cn("h-9 sm:h-8 w-full sm:w-[110px] text-xs border-dashed bg-background",
                                        priority === 'HIGH' ? "text-red-600 bg-red-50 border-red-200" :
                                            priority === 'MEDIUM' ? "text-orange-600 bg-orange-50 border-orange-200" : "text-muted-foreground"
                                    )}>
                                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 w-full lg:w-auto mt-2 lg:mt-0 pt-2 lg:pt-0 border-t lg:border-t-0">
                            ...
                            <Button onClick={handleSubmit} disabled={loading} size="sm" className="h-8 text-xs font-medium px-4 min-w-[100px] flex-1 lg:flex-none">
                                {loading ? (
                                    <>
                                        <IconLoader className="w-3.5 h-3.5 animate-spin mr-2" />
                                        ...
                                    </>
                                ) : (
                                    <>
                                        Créer <IconArrowRight className="w-3.5 h-3.5 ml-1.5 opacity-70" />
                                    </>
                                )}
                            </Button>
                        </div>
...
// ------------------------------------------------------------------
// COMPONENT 2: ASSETS MANAGER (Responsive Grid)
// ------------------------------------------------------------------
function AssetsManager({ projectId }: any) {
    const [file, setFile] = useState<File | null>(null);
    const [uploadAsset, { loading }] = useMutation(UPLOAD_ASSET_MUTATION, {
        onCompleted: () => { toast.success("Fichier uploadé avec succès !"); setFile(null); },
            ...
            <Card className="lg:col-span-2 flex flex-col h-[400px] lg:h-auto">
                <CardHeader className="border-b bg-muted/5 py-4">
                    <CardTitle className="text-base flex items-center gap-2">
                        <IconFiles className="w-4 h-4" /> Fichiers du projet
                    </CardTitle>
                </CardHeader>
                <ScrollArea className="flex-1 p-4">
                    <div className="space-y-2">
                        {/* Mock Data Example - Replace with real data map */}
                        <div className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/30 transition-colors group">
                            <div className="flex items-center gap-3 min-w-0">
                                <div className="h-10 w-10 bg-red-100 text-red-600 rounded flex items-center justify-center shrink-0">
                                    <span className="text-xs font-bold">PDF</span>
                                </div>
                                <div className="min-w-0">
                                    <p className="text-sm font-medium text-foreground truncate">Cahier des charges.pdf</p>
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                                        <span>2.4 MB</span>
                                        <span className="hidden sm:inline">•</span>
                                        <span className="hidden sm:inline">Ajouté le 12 Dec</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="text-center py-10 text-muted-foreground text-sm italic opacity-60">
                            La liste des fichiers sera disponible ici prochainement.
                        </div>
                    </div>
                </ScrollArea>
            </Card>
...
// ------------------------------------------------------------------
// COMPONENT 3: TEAM MANAGER (Responsive Grid)
// ------------------------------------------------------------------
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <TeamSelectionCard
                    ...
                    allUsers={allUsers.infographistes}
                ...
                <TeamSelectionCard
                    ...
                    selectedIds={teamData.coordinatorIds}
                    onChange={handleTeamChange}
                />
            </div>
        </div>
                    ...
                    <div className="space-y-2">
                        {allUsers?.map((user: any) => {
                            ...
                            return (
                                <div
                                    key={user.id}
                                    onClick={() => onChange(role, user.id, !isSelected)}
                                    className={cn(
                                        "flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all hover:shadow-sm select-none",
                                        isSelected ? "bg-primary/5 border-primary/50 shadow-sm" : "bg-background border-border hover:bg-muted/40"
                                    )}
                                >
                                    <div className={cn("w-5 h-5 rounded border flex items-center justify-center transition-colors shrink-0", isSelected ? "bg-primary border-primary" : "border-muted-foreground/30 bg-background")}>
                                        {isSelected && <IconCheck className="w-3 h-3 text-white" />}
                                    </div>
                                    <Avatar className="h-8 w-8 shrink-0">
                                        <AvatarFallback className="text-[10px] font-bold">{user.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                                    </Avatar>
...
// ------------------------------------------------------------------
// COMPONENT 4: KANBAN BOARD (Responsive Grid)
// ------------------------------------------------------------------
...
function TaskCard({ task }: { task: any }) {
    return (
        <Card className="shadow-sm hover:shadow-md transition-all cursor-pointer border-l-4 border-l-transparent hover:border-l-primary group">
            <CardContent className="p-4 space-y-3">
                <div className="flex justify-between items-start gap-2">
                    <p className="text-sm font-medium leading-snug line-clamp-2 text-foreground group-hover:text-primary transition-colors">{task.description}</p>
                    {task.department && (
                        <Badge variant="secondary" className="shrink-0 text-[9px] px-1.5 py-0 h-5 font-normal tracking-wide">
                            {task.department}
                        </Badge>
                    )}
                </div>
