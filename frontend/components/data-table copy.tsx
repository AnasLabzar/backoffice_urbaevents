...
import {
  ...
  IconUsers, IconPlus,
  IconCheck, IconChevronsUpDown // <-- ZIDNA HADO
...
function TaskStatusPill({ status }: { status: string }) {
  let dotColor = "";
  switch (status) {
    case 'TODO': dotColor = "bg-yellow-500"; break;
    case 'IN_PROGRESS': dotColor = "bg-blue-500"; break;
...
const FileStatusCell = ({ row, docType }: { row: any, docType: string }) => {
  ...
  if (file && file.fileUrl) {
    return (
      <a
        href={`http://localhost:5001/${file.fileUrl}`} // L-Link l-s7i7 l-l-backend
        target="_blank"
...
export function DataTable({
  ...
  React.useEffect(() => {
    ...
    if (userRole === 'PROPOSAL_MANAGER') {
      ...
      table.getColumn('tasks_checklist')?.toggleVisibility(false);
...
function TaskChecklistPanel({ project }: { project: Project }) {
  ...
  // N-checkiw chkon m-connecté
  const { data: meData } = useQuery(ME_QUERY);
  const currentUserId = meData?.me.id;

  // --- L-BDIL: Ghadi nst3mlo useQuery 3adi bach y-load l-tasks dima ---
  const { data: taskData, loading: taskLoading } = useQuery(GET_TASKS_BY_PROJECT_QUERY, {
    variables: { projectId: project.id },
    skip: !isOpen, // Ma tjbed 7ta y-clicki
  });
  // -----------------------------------------------------------

  // L-Mutation dyal l-update
  ...
  const handleTriggerClick = () => {
    setIsOpen(true);
    // Ma 7tajinach n3iyto l-getTasks 7it useQuery ghadi ykhdem bo7do
  };
  ...
  // N-filtriw l-tasks (N-affichiw ghir l-tasks dyal l-user li m-connecté)
    ...
    <div className="flex flex-col gap-4 overflow-y-auto px-4 text-sm">
      <div className="flex flex-col gap-3 rounded-lg border p-4">
        ...
        <div className="max-h-64 overflow-y-auto">
          ...
          {/* N-bdloha b Table bach tkon 7ssen */}
          <Table>
            <TableHeader>
              <TableRow>
...
function TableCellViewer({ item }: { item: Project }) {
  ...
  // --- HADI HTA HIYA KANT NAQSSA (DYAL YASSMIN) ---
  ...
  const renderPanelContent = () => {
    ...
    if (userPermissions.includes('manage_assigned_projects') && isToPrepare) {
      return (
        <div className="flex flex-col gap-4 rounded-lg border p-4">
          <h4 className="font-semibold">Étape: Estimation de Coût</h4>
          <p className="text-xs text-muted-foreground">
      ...
      return (
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="flex flex-col gap-4 rounded-lg border p-4">
            ...
            <Button onClick={handleAssignTeam} disabled={loadingTeam} size="sm" className="mt-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    // N-detectiw l-department bo7dna
                ...
                >
                  ...
                  <SelectContent>
                    {/* Nst3mlo l-lista l-jdida (L-LOGIC 4) */}
  ...
  return (
    <Drawer direction={isMobile ? "bottom" : "right"}>
      ...
      <DrawerContent className={cn(
        !isMobile && "sm:max-w-3xl" // Hna 3rrdnah l- 3xl (896px)
      )}>
        <DrawerHeader className="gap-1">
